...
public interface UVRadiationTrackRepository extends CrudRepository<Track, Integer> {
   ...
   @Query(value = "SELECT r FROM Track r WHERE r.fechaServidor >= :fechaInferior AND r.fechaServidor<=:fechaSuperior AND r.uvi IS NOT NULL")//trae todos los trak que esten entre unas fechas
    ...
    //lectura
   @Query(value = "SELECT r FROM Track r WHERE r.fechaServidor >= :fechaInferior AND r.fechaServidor<=:fechaSuperior AND r.lectura IS NOT NULL")//trae todos los trak que esten entre unas fechas
